...
* The panic cannot be caught if _either_ `panic = abort` or the "coredump" crate is enabled, even when the former did not result in generating a core file (e.g., in the integration test).  This is consistent with the https://doc.rust-lang.org/std/panic/fn.catch_unwind.html[documentation for catch_unwind], which says that it does not catch panics when `panic` is not `unwind`.  It's a little surprising that the panic cannot be caught when `panic = "unwind"` and the "coredump" crate is enabled.
